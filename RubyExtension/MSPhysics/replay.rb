  ...
  module Replay
    ...
    class << self
      ...
      def start(activate_animation = true)
        ...
        @layers_data.each { |layer, data|
          if layer.is_a?(Sketchup::Layer) && layer.valid?
            ...
            data[:original][:color] = layer.color if Sketchup.version.to_i >= 14
          end
        }
      ...
      def reset
        ...
        if false # MODERNIZATION: Removed SU2013- compatibility
        ...
        @layers_data.each { |layer, data|
          ...
          if Sketchup.version.to_i >= 14 && layer.color.to_i != orig_data[:color].to_i
      ...
      def record_layer(layer, pframe)
        ...
        data[pframe][:color] = layer.color if Sketchup.version.to_i >= 14
        update_data_frame_limits(data, pframe)
      end
      ...
      def activate_frame(pframe)
        ...
        if @replay_layers
          @layers_data.each { |layer, data|
            ...
            if Sketchup.version.to_i >= 14 && frame_data[:color] && layer.color.to_i != frame_data[:color].to_i
