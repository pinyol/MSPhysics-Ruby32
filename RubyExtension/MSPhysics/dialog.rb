...
module MSPhysics
  ...
  module Dialog
    ...
    # MODERNIZATION: Enable HtmlDialog for SketchUp 2017+ (Ruby 3.2 support)
    # Changed from: USE_HTML_DIALOG = false
        ...
        if state
          ...
          @dialog.add_action_callback('editor_changed') { |acs, params|
            if @selected_body && @selected_body.valid?
              # MODERNIZATION: Simplified for HtmlDialog - params works for both dialog types
              code = params
            ...
            when 'body-assign_props_to_all'
              if @selected_body && @selected_body.valid? && (@selected_body.name.size > 0 || ::UI.messagebox("This body is unnamed. Would you like to assign identical properties to all the unnamed bodies?", MB_YESNO) == IDYES)
                ...
                if dict
                  ...
                  model.start_operation(op, true, false, false)
          ...
          @dialog.add_action_callback('numeric_input_changed') { |acs, params|
            ...
            when 'body'
              if @selected_body && @selected_body.valid?
                ...
                @selected_body.set_attribute('MSPhysics Body', option, value.to_i.is_a?(Integer) ? value.to_s : value)
              end
            when 'joint', *MSPhysics::JOINT_FILE_NAMES
              if @selected_joint && @selected_joint.valid?
                ...
                @selected_joint.set_attribute('MSPhysics Joint', option, value.to_i.is_a?(Integer) ? value.to_s : value)
              end
            when 'gear'
              if @selected_joint && @selected_joint.valid?
          ...
          @dialog.add_action_callback('placement_changed') { |acs, params|
            ...
            wsx = nil
            wsy = nil
            ui_scale = Sketchup.version.to_i >= 17 ? ::UI.scale_factor : 1.0
